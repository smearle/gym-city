...
parser.add_argument('--env-name', type=str, default='MicropolisWalkEnv-v0')
...
env = gym.make('MicropolisWalkEnv-v0')
MAP_X, MAP_Y = 48, 48
STEP_DIST = 6
env.setMapSize(MAP_X, MAP_Y, STEP_DIST)
# np.random.seed(1234)
# env.seed(1234)
...
model.add(Convolution2D(32, (3, 3), strides=(1, 1), padding='valid'))
model.add(Activation('relu'))
model.add(Convolution2D(num_tools, (7, 7)))
model.add(Activation('relu'))
model.add(Flatten())
model.add(Dense(512))
...
model.add(Activation('relu'))
model.add(Conv2DTranspose(num_tools, (5, 5), strides=(1, 1)))
