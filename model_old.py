...
class FractalBlock(NNBase):
    def __init__(self, num_inputs, recurrent=False, hidden_size=512,
                 map_width=16, n_recs=3, rule='extend',
                 num_actions=19,  n_chan=32,
                 n_chan_in=32, base=None, squeeze=False, shared=False):
        ...
        self.intercol_share = False # share weights between columns
...
class SubFractal(nn.Module):
    def __init__(self, root, f_c, n_layer, conv=None, n_conv_recs=2):
        ...
        if f_c is not None:
            ...
            self.f_c_B.fixed = init_(nn.Conv2d(self.n_chan, self.n_chan,
            3, 1, 1))
        else:
            self.f_c_B = f_c
