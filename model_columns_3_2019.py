...
class MicropolisBase_fractal(NNBase):
    def __init__(self, num_inputs, recurrent=False, hidden_size=512, 
        ...
        self.SHARED = True # if true, weights are shared
        ...
        for i in range(n_unique_lyrs):
            ...
            if squeeze and (n_unique_lyrs == 1 or i > 0):
            ...
            if i > 0:
                setattr(self, 'join_{}'.format(i), init_(nn.Conv2d(self.n_channels * 2, self.n_channels, 3, 1, 1)))
    ...
    def applyRepeat(self, subnet, skip):
        ''' Returns a network comprising a 2-chain of subnet, and skip, in parallel.'''
        pass
    ...
    def forward(self, x, rnn_hxs, masks):
        ...
        if not self.COLUMNS and not self.global_drop:
            # (column, join depth)
...
class MicropolisBase_fractal_recurrent(NNBase): # incomplete
    def __init__(self, num_inputs, recurrent=True, hidden_size=512, 
        ...
        self.map_width = map_width
        self.num_maps = int(math.log(map_width, 2) - 1)
        self.n_channels = 32
        self.n_recs = n_recs
        self.depth = pow(2, self.n_recs - 1)
        print("Fractal Net: ", self.n_recs, "recursions, ",squeeze, "squeeze")
        self.n_conv_recs = n_conv_recs
        self.squeeze = squeeze
        self.COLUMNS = True
        ...
        for i in range(1):
            fixed =  nn.Sequential(
                        nn.Conv2d(self.n_channels, self.n_channels, 3, 1, 1)
                        )
            fixed = ConvLSTMCell(self.n_channels, self.n_channels)
            fixed.apply(seq_init_)
            setattr(self, 'fixed_{}'.format(i), fixed)
            if squeeze: #and i > 0:
                setattr(self, 'dwn_{}'.format(i), init_(nn.Conv2d(self.n_channels, self.n_channels, 2, 2, 0)))
                setattr(self, 'up_{}'.format(i), init_(nn.ConvTranspose2d(self.n_channels, self.n_channels, 2, 2, 0)))
        ...
        if not self.COLUMNS:
            ...
            if self.global_drop:
                ...
                for i in range(depth // (2 ** j)):
                   #active_column = getattr(self, 'col_{}'.format(j))
                   #x = active_column(x)
                    fixed = getattr(self, 'fixed_{}'.format(0))
                    if self.squeeze:
                        if j > 0:
                       ...
                       #n_squish = j
                        ...
                        for k in range(n_squish):
                            ...
                            for l in range(k + 3):
                                hidden = rnn_hxs[j][c]
                                c += 1
                                x = rnn_hxs[j][c] = fixed(x), hidden
                                x = x[0]
                                x = F.relu(x)
                        hidden = rnn_hxs[j][c]
                        c += 1
                        x, rnn_hxs[j][c] = F.relu(fixed(x), hidden)
                        for k in range(n_squish):
                        ...
                        hidden = rnn_hxs[j][0]
                        x, rnn_hxs[j][0] = F.relu(fixed(x, hidden))
            else:
                for r in range(self.n_recs):
                    globals()['in_{}'.format(r)] = x
                for i in range(depth):
                   #print('depth ', i)
                    ...
                    while (i + 1) % (2 ** j) == 0 and j < self.n_recs:
                        ...
                        if not self.local_drop or self.join_masks[i][j]:
                           #column = getattr(self, 'col_{}'.format(j))
                            x_j = globals()['in_{}'.format(j)]
                            fixed = getattr(self, 'fixed_{}'.format(0))
                            c = 0 # num recurrent layers in column
                            if self.squeeze:
                                if j > 0:
                                    dwn = getattr(self, 'dwn_{}'.format(0))
                                    up = getattr(self, 'up_{}'.format(0))
                                n_squish = min(self.num_maps, j)
                                for k in range(n_squish):
                                    x_j = F.relu(dwn(x_j))
                                    for l in range(k + 3):
                                        hidden = rnn_hxs[j][c]
                                        x_j = rnn_hxs[j][c] = fixed(x_j, hidden)
                                        x_j = x_j[0] # of (hidden, cell) take the former
                                        x_j = F.relu(x_j)
                                        c += 1
                                hidden = rnn_hxs[j][c]
                                c += 1
                                ...
                                hidden = rnn_hxs[j][0]
                                x_j = rnn_hxs[j][0] = fixed(x_j, hidden)
                                x_j = F.relu(x_j[0])
                                x_j = F.relu(x_j)
                           #x_j = column(globals()['in_{}'.format(j)])
                           #out = (out * a + x_j) / (a + 1)
                            if out is None:
                                out = x_j
                            else:
                                out = F.relu(self.join_compress(torch.cat((out, x_j), 1)))
                            a += 1
                       #print('depth {} joined w/ col {}'.format(i,j))
                        j += 1
                    for k in range(j):
    ...
    def get_initial_recurrent_states(self):
        ''' '''
