    ...
    def __init__(self,
        ...
        if type(recurrent_hidden_state_size) is tuple:
            self.recurrent_hidden_states = torch.zeros(
                num_steps + 1, 2, num_processes, *recurrent_hidden_state_size)
        else:
            self.recurrent_hidden_states = torch.zeros(
            ...
            if action_space.__class__.__name__ == 'Dict':
                ...
                for name, space in action_space.spaces.items():
                    if space.__class__.__name__ == 'Discrete':
                        ...
                        for k in space.shape:
                            action_shape += k
                    self.actions[name] = torch.zeros(num_steps, num_processes,
                                                     action_shape)
                    self.action_log_probs[name] = torch.zeros(
    ...
    def to(self, device):
        ...
        if isinstance(self.actions, dict):
            for k in self.actions:
                self.actions[k].to(device)
                self.action_log_probs[k].to(device)
        else:
...
class CuriosityRolloutStorage(RolloutStorage):
    def __init__(
        ...
        self.action_bins = torch.zeros(num_steps, num_processes,
                                       action_space.n)
        self.action_dist_preds = torch.zeros(num_steps + 1, num_processes,
                                             action_space.n)
        self.feature_states = torch.zeros(num_steps, num_processes,
                                          *state_feature_space)
        self.feature_state_preds = torch.zeros(num_steps + 1, num_processes,
                                               *state_feature_space)
    ...
    def insert(self, obs, recurrent_hidden_states, actions, action_log_probs,
               value_preds, rewards, masks, feature_state, feature_state_pred,
               action_bin, action_dist_pred):
