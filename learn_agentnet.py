...
# suppose this convolution maintains binary zone info on input, and adds binary info about road connectivity, power, and desirability on recurrence
conv0 = Conv2DLayer(in_conv0, num_zones + 3, 1, stride=1, nonlinearity=tanh, pad='same')
...
conv_rec = Conv2DLayer(in_conv_rec, num_zones + 3, 3, stride=1, nonlinearity=tanh, pad='same')
...
# map
conv_mapwide = Conv2DLayer(conv4, num_tools, MAP_X, stride=1, nonlinearity=elu, pad="same")
print(get_output_shape(conv_mapwide))

qvalues_layer = reshape(simple_conv, ([0],-1))
